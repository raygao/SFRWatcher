...
class Salesforce::ChatterFeed < ActiveRecord::Base

  set_table_name 'chatter_feeds'
    ...
    begin
      ...
      if results.queryResponse.result.records.is_a?(Hash)
        ...
        if (!feed.FeedPost.nil?) && (feed.FeedPost.ContentSize.to_i > 0)
          feed = get_single_chatter_feed_with_attachment(feed.FeedPost.Id[0], feed[:type], binding, directory_name)
        end
        feed_results << feed
        ...
        results.queryResponse.result.records.each do |an_feed_entry|
          if (an_feed_entry.is_a?(Hash))
            if (an_feed_entry[:Type] == "TrackedChange")
              ...
              an_entry_with_attachment = get_single_chatter_feed_with_attachment(an_feed_entry.FeedPost.Id[0], an_feed_entry[:type], binding, directory_name)
  ...
  def get_single_chatter_feed_with_attachment(feedpost_id, feed_type, binding, directory_name, limit=100)
    begin
      ...
      if !deep_result.Fault.nil?
        raise ChatterFeedError.new(deep_result.Fault.faultstring.to_s)
      end
      ...
      if filename.nil?
        ...
        local_file_name = file_writer(filename, filesize, filedata, directory_name)
